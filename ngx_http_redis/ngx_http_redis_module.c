...
ngx_http_redis_handler(ngx_http_request_t *r)
{
    ...
    ngx_http_redis_uri_config *c = uri_configs;
    while(1){
	    if( NULL == c->uri ) {
		    return NGX_DECLINED;	
	    } else {
		    if( 0 == strncmp( (char*) r->uri.data,c->uri,c->compare_len)  ) {
			    break;
		    }
	    }
	    c++;
...
ngx_http_redis_create_request(ngx_http_request_t *r)
{
    ...
    while(1){
        if( NULL == c->uri ) {
            ...
            if( 0 == strncmp( (char*)vv[1]->data,c->uri,c->compare_len)  ) {
                vv[1]->len += c->len_range;
                vv[1]->data += c->key_data_range;
                break;
            }
        }
    	c++;
...
found:
    ...
    *p = '\0';
    ...
    if (ngx_strncmp(p, "$", sizeof("$") - 1) == 0) {
        ...
        ssize_t data_len = 0;
        ssize_t len_len = p - len - 1;
        u_char *data_last;
        int shm_id = 0xff12000;
	  ...
	  c++;
        ...
        if( NGX_OK != ngx_http_redis_rebuild_response(len,len_len,&data_len,&data_last,shm_id,r->connection->log)){
    		ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_http_redis_module rebuild content failed " );
            return NGX_ERROR; 
        }
        u->headers_in.content_length_n = data_len;
        u->buffer.last = data_last;
...
no_valid:
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "redis sent invalid response: \"%V\"", &line);
    return NGX_HTTP_UPSTREAM_INVALID_HEADER;
}
...
ngx_http_redis_create_loc_conf(ngx_conf_t *cf)
{
    ...
    conf->upstream.buffer_size =  ngx_http_redis_buffer_size;
...
int ngx_http_redis_rebuild_response(u_char *len_pos, ssize_t len_len, ssize_t *data_len, u_char **data_last, int shm_id, ngx_log_t *log) { 
	...
	cJSON *json,*item;
    int index = 0;
    char result[ngx_http_redis_buffer_size] = "Default Content\n";
    char num_area[640] = {'\0'},*tmp = num_area;
    ssize_t len;
    u_char *end;
    char *replace_args[512];

    while(*data_begin && *data_begin++ != CR);
    data_begin++;

    json=cJSON_Parse((char *)data_begin);
	
	if( NULL == json) {
        ngx_log_error(NGX_LOG_ERR, log, 0, "json parse failed.");
        return NGX_ERROR; 
	}
    ...
    if(json->type != cJSON_Object) {
        ngx_log_error(NGX_LOG_ERR, log, 0, "json type invalid .");
        return NGX_ERROR; 
    }
    item = (NULL == json->child) ? json : json->child;
    while (item) {
		if( index >= 256 ) {
        	ngx_log_error(NGX_LOG_ERR, log, 0, "Too many variables . Some variables has been ignored.");
			break;
		}
        replace_args[index * 2] = item->string;
        switch(item->type){
            case 0:
            case 1:
            case 2:
            case 3:
				if( 640 <= (tmp - num_area)) {
        			ngx_log_error(NGX_LOG_ERR, log, 0, "Too many numbers. Some numbers has been ignored.");
					break;	
				}
                if(item->valuedouble != item->valueint){
                    sprintf(tmp, "%g", item->valuedouble);
                } else{
                    sprintf(tmp, "%d", item->valueint);
                }   
                replace_args[index * 2 + 1] = tmp;
                tmp += 32;
                break;
            case 4:
                replace_args[index * 2 + 1] = item->valuestring;
                break;
            case 5:
            case 6:
            default: break;
    ...
    if( -1 == ( shmid = shmget(shm_id, 0, 0 ))) {
        ngx_log_error(NGX_LOG_ERR, log, 0, "shmget failed .");
        return NGX_ERROR;
    }
    ...
    if( NGX_OK != ngx_http_redis_parse_tpl(result,replace_args,index * 2,shm_id) ){
        ngx_log_error(NGX_LOG_ERR, log, 0, "parse tpl filed. .");
        return NGX_ERROR; 
    }

    len = strlen(result);

    sprintf((char*)len_pos,"%ld\r%c%s\r\n",len-1,'\0',result);
...
int ngx_http_redis_parse_tpl(char* buf,char** replace_argv,int replace_argc,int shm_id){
    int i = 0;
	for(;i < (replace_argc - 1) ; i += 2){
        ngx_http_redis_replace_str(buf,replace_argv[i],replace_argv[i+1]);	
    }
    return NGX_OK;
}

void ngx_http_redis_replace_str(char* content, char *val_name, char *value){
    char buffer[ngx_http_redis_buffer_size]  = {'\0'};
    char *p; 
    do{ 
        bzero(buffer,ngx_http_redis_buffer_size);
        if(!(p = strstr(content, val_name)) || 0 == strcmp(val_name, value)){
            break;
        }   
        strcpy(buffer,p+strlen(val_name)); 
        sprintf(p,"%s%s",value,buffer);
    } while(p);
}
