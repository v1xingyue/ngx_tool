...
static int rebuild_response(u_char *,ssize_t,ssize_t*,u_char **,int);
...
ngx_http_redis_create_request(ngx_http_request_t *r)
{
    ...
    if( 0 == strncmp((char*)vv[1]->data,"/home",5) ) {
        vv[1]->len -= 1;
        vv[1]->data += 1;
    } else if (vv[1]->len > 8 ){
        vv[1]->len -= 8;
...
found:
    ...
    *p = '\0';
    ...
    if (ngx_strncmp(p, "$", sizeof("$") - 1) == 0) {
        ...
        if( 0 == strncmp( (char*)r->uri.data,"/home",5 )  ) {
            shm_id = 0xff13000; 
        }

        if( 0 != rebuild_response(len,len_len,&data_len,&data_last,shm_id)){
...
ngx_http_redis_create_loc_conf(ngx_conf_t *cf)
{
    ...
    conf->upstream.buffer_size = 1024 * 1024 * 1024;
...
int rebuild_response(u_char *len_pos, ssize_t len_len, ssize_t *data_len, u_char **data_last, int shm_id) { 
   ...
   cJSON *json,*json_child,*name_json,*value_json;
   int index = 0, array_size = 0;
   char result[409600] = "Hello world.\n";
   ssize_t len;
   u_char *end;
   char *replace_args[500];

   json=cJSON_Parse((char *)data_begin);
   array_size = cJSON_GetArraySize(json);
  
   for(;index< array_size ; index++){
        json_child = cJSON_GetArrayItem(json,index);
        name_json = cJSON_GetObjectItem(json_child,"name");
        value_json = cJSON_GetObjectItem(json_child,"value");
        replace_args[index * 2] = name_json->valuestring;
        replace_args[index * 2 + 1] = value_json->valuestring;
   }

   if( 0 != parse_file(result,replace_args,index * 2,shm_id) ){
        return -1; 
   }
...
int parse_file(char* buf,char** replace_argv,int replace_argc,int shm_id){
	int i = 0;
    char *shmptr;
    int shmid;
