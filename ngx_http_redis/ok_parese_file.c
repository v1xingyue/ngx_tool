...
static void rebuild_response(u_char *,ssize_t,ssize_t*,u_char **);
...
ngx_http_redis_create_request(ngx_http_request_t *r)
{
    ...
    if (vv[0] == NULL || vv[0]->not_found || vv[0]->len == 0) {
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "select 0 redis database" );
        len = sizeof("select 0") - 1;
    } else {
        len = sizeof("select ") - 1 + vv[0]->len;
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "select %s redis database", vv[0]->data);
    }
    len += sizeof(CRLF) - 1;
    ...
    if (vv[1] == NULL || vv[1]->not_found || vv[1]->len == 0) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "the \"$redis_key\" variable is not set");
        return NGX_ERROR;
    }
    ...
    *b->last++ = 'c'; *b->last++ = 't'; *b->last++ = ' ';
    ...
    if (vv[0] == NULL || vv[0]->not_found || vv[0]->len == 0) {
        ...
        *b->last++ = '0';
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "select %V redis database", &ctx->key);
    }
    ...
    *b->last++ = 'g'; *b->last++ = 'e'; *b->last++ = 't'; *b->last++ = ' ';
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http redis request: \"%V\"", &ctx->key);
...
ngx_http_redis_process_header(ngx_http_request_t *r)
{
    ...
    u = r->upstream;
    /*
    ngx_str_t str = ngx_string("+OK\r\n$48\r\nthis is a test modify in process header fucntion\r\n");
    ngx_memcpy(u->buffer.pos, str.data, str.len);
    u->buffer.last = u->buffer.pos + str.len;
    */
...
found:
    *p = '\0';
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "redis: \"%V\"", &line);
    p = u->buffer.pos;
    ...
    if (ngx_strncmp(p, "$", sizeof("$") - 1) == 0) {
        ...
        rebuild_response(len,len_len,&data_len,&data_last);
...
{
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "abort http redis request");
    return;
}
...
{
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "finalize http redis request");
    return;
}
...
ngx_http_redis_add_variables(ngx_conf_t *cf)
{
    ...
    var = ngx_http_add_variable(cf, &ngx_http_redis_db, NGX_HTTP_VAR_CHANGEABLE);
    if (var == NULL) {
        return NGX_ERROR;
    }
...
void  rebuild_response(u_char *len_pos, ssize_t len_len, ssize_t *data_len, u_char **data_last) { 

   //from len_pos change data. 
   //len_pos + len_len + 2 is the real data segment
   
   //u_char *t = len_pos,*data_begin;
   u_char *data_begin;
   struct test_t p;
   data_begin = len_pos;
   ...
   ngx_memcpy(&p,data_begin, sizeof(struct test_t) ) ;
//   printf(" id %d \n",(int)p.id);
//   printf(" title %s \n",p.title);
...
//
   ...
   parse_file(result,"/usr/home/xingyue/ngx_tool/ngx_http_redis-0.3.7/t.tpl",
        replace_args,8
   );   
    
   len = strlen(result);
   
   sprintf((char*)len_pos,"%d\r%c%s\r\n",len-1,'\0',result);
   ...
   *data_len = len - 1;
   ...
   //ngx_memcpy(len_pos,str.data,str.len);
  
   //last = len_pos + str.len;

   //*data_last = len_pos + str.len;
   //*data_len = 26;   
   //while(*data_last && *data_last++ != '\0') ;
   //data_last--;
}
