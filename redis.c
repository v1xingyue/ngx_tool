...
static void replace_str(u_char *,ssize_t,ssize_t*,u_char **);
...
found:
    *p = '\0';
    ...
    if (ngx_strncmp(p, "$", sizeof("$") - 1) == 0) {
        ...
        u_char *data_last;
        replace_str(len,len_len,&data_len,&data_last);
...
void  replace_str(u_char *len_pos, ssize_t len_len, ssize_t *data_len, u_char **data_last) { 

   //from len_pos change data. 
   //len_pos + len_len + 2 is the real data segment
   u_char *t = len_pos;
   struct test_t p;
   ngx_memcpy(&p,t + len_len + 2, sizeof(struct test_t) ) ;
   printf(" redis value %d \n",(int)p.value);
   *data_len = 26;
   ngx_str_t str = ngx_string("26\r\0ABCDEFGHIJKLMNOPQRSTUVWXYZ\r\n"); 
